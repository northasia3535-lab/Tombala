...
(function () {
  ...
  // Çinko yapıldığında çalan, çağrı sesinden ayrı, kısa yükselen üçlü ton
  function playCinkoFanfare() {
    ...
    [523, 659, 784].forEach((freq, i) => {
      ...
      gain.gain.linearRampToValueAtTime(0.2, now + i * 0.11 + 0.02);
  ...
  // Kazananın tüm kartını doldurduğu anda çalan, daha uzun/zengin final fanfarı
  ...
  function enterRoom(code) {
    ...
    localStorage.setItem("tombala_last_room", code);
  ...
  function leaveRoom() {
    ...
    localStorage.removeItem("tombala_last_room");
  ...
  // Sayfa yenilenirse (telefon ekranı kilitlenmesi, sekme yenileme vb.) daha
  // önce içinde bulunulan odaya sessizce yeniden bağlan — isim/ses/kazanan
  // bilgisinin kaybolmasının en sık sebebi budur.
  function tryAutoRejoin() {
    if (!firebaseReady) return;
    const lastRoom = localStorage.getItem("tombala_last_room");
    const savedName2 = localStorage.getItem("tombala_name");
    if (!lastRoom || !savedName2) return;
    db.ref(`rooms/${lastRoom}`).once("value").then(snap => {
      if (!snap.exists()) { localStorage.removeItem("tombala_last_room"); return; }
      const room = snap.val();
      if (!room.players || !room.players[myId]) { localStorage.removeItem("tombala_last_room"); return; }
      myName = room.players[myId].name || savedName2;
      enterRoom(lastRoom);
    }).catch(() => { /* no-op */ });
  }
  ...
  function renderRoom(room) {
    ...
    // myName değişkeni sayfa yenilemesinde boşalabilir; sunucudaki isimle
    // her render'da senkronize tutarak çinko/kazanan duyurularının doğru
    // ismi göstermesini garanti ederiz.
  ...
  // torbayı görebilir; oda sahibi yeni oyunu başlatana kadar sadece
  // ekranımızdan kapatılır, oyun durumu değişmez.
  ...
  // Sayılar artık torbadan açılınca kendiliğinden karta yerleşiyor
  // (bkz. autoMarkCalledNumber). Elle dokunma, olası bir gecikme/hata
  // durumunda yedek olarak duruyor — zaten işaretliyse veya torbadan
  // çıkmamışsa hiçbir şey yapmaz.
  function markNumberForMe(val, cell, me, room) {
    flyNumberToCell(val, cell);
    const path = `rooms/${currentRoomCode}/players/${myId}`;
    db.ref(`${path}/marked/${val}`).set(true).then(() => checkRowsAndMaybeWin(room, me, val));
  }
  ...
  // Oda sahibi bir sayı açtığı anda, o sayı bizim kartımızda varsa
  // dokunmaya gerek kalmadan otomatik olarak uçup yerleşir — böylece
  // sayı gözden kaçıp kaçırılmış olmaz.
  function autoMarkCalledNumber(room, me) {
    if (!room || room.status !== "playing" || !me || !me.card) return;
    const curIdx = room.currentCallIndex;
    if (curIdx === null || curIdx === undefined) return;
    if (curIdx === lastAutoMarkedIdx) return;
    lastAutoMarkedIdx = curIdx;

    const order = room.numberOrder || [];
    const val = order[curIdx];
    if (val === undefined) return;
    const marked = me.marked || {};
    if (marked[val]) return; // zaten işaretli

    let targetCell = null;
    for (let r = 0; r < 3 && !targetCell; r++) {
      for (let c = 0; c < 9; c++) {
        if (me.card[r][c] === val) {
          const wrap = $("myCard");
          targetCell = wrap.children[r * 9 + c];
          break;
        }
      }
    }
    if (!targetCell) return; // sayı bu kartta yok

    // uçuş animasyonunun kart hücrelerinin gerçek konumunu okuyabilmesi
    // için bir sonraki çizim karesini bekle
    requestAnimationFrame(() => {
      markNumberForMe(val, targetCell, me, room);
    });
  }
  ...
  function announceCinko() {
    // Sunucudaki (Firebase'deki) isim, yerel `myName` değişkeninden daha
    // güvenilir — sayfa yenilemesinden etkilenmez.
    const myServerName = (lastRoomSnapshot && lastRoomSnapshot.players && lastRoomSnapshot.players[myId] && lastRoomSnapshot.players[myId].name) || myName || "Oyuncu";
    ...
    counterRef.transaction(cur => (cur || 0) + 1).then(res => {
      if (res.committed) {
        ...
        db.ref(`rooms/${currentRoomCode}/cinkoLog/${n}`).set({ n, playerId: myId, playerName: myServerName, ts: Date.now() });
      }
    });
  }
  ...
  function renderCinkoAndWinner(room, players) {
    ...
    if (count > lastCinkoCount) {
      for (let n = lastCinkoCount + 1; n <= count; n++) {
        ...
        if (entry) {
          ...
          const playerLabel = entry.playerName || (players[entry.playerId] && players[entry.playerId].name) || "Bir oyuncu";
          if (!isFull) {
            toast(`🎉 ${n}. Çinko — ${playerLabel}!`);
            playCinkoFanfare();
            speakText(`${n}. Çinko, ${playerLabel} yaptı!`);
          }
        }
      }
    ...
    const winnerLabel = room.winnerName || (room.winnerId && players[room.winnerId] && players[room.winnerId].name) || "Bilinmiyor";
    if (room.status === "finished" && room.winnerId) {
      const isFirstShow = !winOverlayShown;
