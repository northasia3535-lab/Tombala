...
(function () {
  ...
  function speakNumber(n) {
    try {
      ...
      const u = new SpeechSynthesisUtterance(numberToTurkish(n));
      u.lang = "tr-TR";
      u.rate = 0.95;
  ...
  function onCardCellClick(e) {
    ...
    // uçuş animasyonu -> işaretle -> satır/kart kontrolü
  ...
  function renderCinkoAndWinner(room, players) {
    ...
    if (count > lastCinkoCount) {
      for (let n = lastCinkoCount + 1; n <= count; n++) {
        ...
        if (entry) {
          ...
          if (!isFull) toast(`🎉 ${n}. Çinko — ${entry.playerName}!`);
        }
      }
    ...
    if (room.status === "finished" && room.winnerId) {
      $("winnerName").textContent = room.winnerName || "Bilinmiyor";
      const iAmOwner = room.ownerId === myId;
